...
def plot_trajectory(records, output_path: str):
    ...
    vel_limits_arr = np.array([ARM_JOINT_VEL_LIMIT[j] for j in ARM_JOINT_NAMES])
    accel_limits_arr = np.array([ARM_JOINT_ACCEL_LIMIT[j] for j in ARM_JOINT_NAMES])
 
    # Chuẩn hoá mỗi khớp theo ĐÚNG giới hạn CỦA RIÊNG NÓ (không phải giới hạn
    # chặt nhất chung) - vì các khớp có vel_limit/accel_limit KHÁC NHAU
    # (foreArm_joint thấp hơn 5 khớp còn lại), dùng chung 1 đường ngưỡng cho
    # cả 6 đường sẽ khiến khớp có giới hạn cao hơn "trông như vi phạm" dù
    # thực ra vẫn hoàn toàn đúng giới hạn của chính nó.
    qdot_cmd_norm = qdot_cmd / vel_limits_arr[np.newaxis, :]
    qddot_cmd_norm = qddot_cmd / accel_limits_arr[np.newaxis, :]
 
    fig, axes = plt.subplots(3, 1, figsize=(11, 13), sharex=True)
 
    for i, jname in enumerate(ARM_JOINT_NAMES):
        axes[0].plot(t, q_cmd[:, i], label=jname)
        axes[1].plot(t, qdot_cmd_norm[:, i], label=jname)
        axes[2].plot(t, qddot_cmd_norm[:, i], label=jname)
 
    axes[0].set_ylabel("Vị trí khớp q_cmd (rad)")
    axes[0].set_title("Quỹ đạo vị trí khớp đã ra lệnh (q_cmd) theo thời gian")
    axes[0].legend(loc="center left", bbox_to_anchor=(1.01, 0.5), fontsize=8, borderaxespad=0.0)
    axes[0].grid(True, alpha=0.3)
 
    axes[1].axhline(1.0, color="red", linestyle="--", linewidth=1, label="100% giới hạn (của từng khớp)")
    axes[1].axhline(-1.0, color="red", linestyle="--", linewidth=1)
    axes[1].set_ylim(-1.15, 1.15)
    axes[1].set_ylabel("qdot_cmd / vel_limit (khớp đó)")
    axes[1].set_title("Vận tốc đã ra lệnh, CHUẨN HOÁ theo giới hạn riêng từng khớp - không bao giờ vượt ±1.0")
    axes[1].legend(loc="center left", bbox_to_anchor=(1.01, 0.5), fontsize=8, borderaxespad=0.0)
    axes[1].grid(True, alpha=0.3)
 
    axes[2].axhline(1.0, color="red", linestyle="--", linewidth=1, label="100% giới hạn (của từng khớp)")
    axes[2].axhline(-1.0, color="red", linestyle="--", linewidth=1)
    axes[2].set_ylim(-1.15, 1.15)
    axes[2].set_ylabel("qddot_cmd / accel_limit (khớp đó)")
    axes[2].set_xlabel("Thời gian (giây)")
    axes[2].set_title("Gia tốc đã ra lệnh, CHUẨN HOÁ theo giới hạn riêng từng khớp - không bao giờ vượt ±1.0")
    axes[2].legend(loc="center left", bbox_to_anchor=(1.01, 0.5), fontsize=8, borderaxespad=0.0)
